...
use crate::state::{REPLY_ID_TO_USER_CHAIN_REGISTRATION, user_chain_registrations, UserChainRegistration};
...
pub fn reply(deps: DepsMut, _: Env, msg: Reply) -> StdResult<Response> {
    ...
    let reply_id_to_reg = REPLY_ID_TO_USER_CHAIN_REGISTRATION.may_load(deps.storage, msg.id).unwrap();
    if reply_id_to_reg.is_some() {
        user_chain_registrations().update(deps.storage, reply_id_to_reg.unwrap(), |reg_opt| -> Result<UserChainRegistration, StdError> {
   ...
   let regs: Vec<_> = user_chain_registrations().range(deps.storage, None, None, cosmwasm_std::Order::Ascending)
        .collect(); 

    let reply_regs: Vec<_> = REPLY_ID_TO_USER_CHAIN_REGISTRATION.range(deps.storage, None, None, cosmwasm_std::Order::Ascending)
        .collect();
