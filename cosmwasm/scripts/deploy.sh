...
echo "neutrond q wasm contract-state smart $CONTRACT_ADDR '{\"\": {\"due_user_chain_registrations_response\": {\"delegators_amount\": 5}}}'"
echo ""
...
echo "neutrond q wasm contract-state smart $CONTRACT_ADDR '{\"supported_chains\": {}}'"
echo "Then, run:"
echo "gaiad tx authz grant INSERT_ICA_ACCOUNT_HERE delegate --allowed-validators $COSMOS_HUB_VAL --from $COSMOS_HUB_USER_KEY --keyring-backend test --gas auto --gas-adjustment 2 --gas-prices 0.025uatom --chain-id test-2 --node tcp://:16657"